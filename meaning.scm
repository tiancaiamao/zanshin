...
;;;---------------------interface---------------------
(define SHALLOW-ARGUMENT-REF (lambda (j) (list 'SHALLOW-ARGUMENT-REF j)))
(define DEEP-ARGUMENT-REF (lambda (i j) (list 'DEEP-ARGUMENT-REF i j)))
(define SET-DEEP-ARGUMENT! (lambda (i j) (list 'SET-DEEP-ARGUMENT! i j)))
(define CHECKED-GLOBAL-REF (lambda (i) (list 'CHECKED-GLOBAL-REF i)))
(define CONSTANT (lambda (v) (list 'CONSTANT v)))
(define GOTO (lambda (offset) (list 'GOTO offset)))
(define UNLINK-ENV (lambda () (list 'UNLINK-ENV)))
(define INVOKE1 (lambda (address) (list 'INVOKE1 address)))
(define POP-ARG1 (lambda () (list 'POP-ARG1)))
(define POP-ARG2 (lambda () (list 'POP-ARG2)))
(define CREATE-CLOSURE (lambda (offset) (list 'CREATE-CLOSURE offset)))
(define RETURN (lambda () (list 'RETURN)))
(define ARITY>=? (lambda (arity+1) (list 'ARITY>=? arity+1)))
(define FUNCTION-INVOKE (lambda () (list 'FUNCTION-INVOKE)))
(define RESTORE-ENV (lambda () (list 'RESTORE-ENV)))
(define POP-CONS-FRAME! (lambda (arity) (list 'POP-CONS-FRAME! arity)))
(define ALLOCATE-DOTTED-FRAME (lambda (arity) (list 'ALLOCATE-DOTTED-FRAME arity)))
(define PREDEFINED (lambda (i) (list 'PREDEFINED i)))
(define SET-SHALLOW-ARGUMENT! (lambda (j) (list 'SET-SHALLOW-ARGUMENT! j)))
(define GLOBAL-REF (lambda (i) (list 'GLOBAL-REF i)))
(define SET-GLOBAL! (lambda (i) (list 'SET-GLOBAL! i)))
(define JUMP-FALSE (lambda (offset) (list 'JUMP-FALSE offset)))
(define EXTEND-ENV (lambda () (list 'EXTEND-ENV)))
(define CALL0 (lambda (address) (list 'CALL0 address)))
(define PUSH-VALUE (lambda () (list 'PUSH-VALUE)))
(define INVOKE2 (lambda (address) (list 'INVOKE2 address)))
(define INVOKE3 (lambda (address) (list 'INVOKE3 address)))
(define ARITY=? (lambda (arity+1) (list 'ARITY=? arity+1)))
(define PACK-FRAME! (lambda (arity) (list 'PACK-FRAME! arity)))
(define POP-FUNCTION (lambda () (list 'POP-FUNCTION)))
(define PRESERVE-ENV (lambda () (list 'PRESERVE-ENV)))
(define POP-FRAME! (lambda (rank) (list 'POP-FRAME! rank)))
(define ALLOCATE-FRAME (lambda (size) (list 'ALLOCATE-FRAME size)))
(define FINISH (lambda () (list 'FINISH)))
